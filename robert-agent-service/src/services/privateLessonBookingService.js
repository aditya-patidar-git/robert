...
class PrivateLessonBookingService extends BaseBookingService {
  constructor() {
    super(
      'Private Lesson', // courseType
      'https://www.bookcbtnow.com/incontact/public/gateway.aspx?func_id=79A2A98E7C95DA57&obc_id=CFB644AFFB83F5C6', // availabilityUrl
      './screenshots/private-lesson-booking' // screenshotsDir
    );
  }
  ...
  // STEP 7: Select booking options for existing client (Private Lesson-specific)
  ...
  // STEP 5: Select booking options for new client (Private Lesson-specific)
  ...
  // Internal method: Select booking options (Private Lesson-specific)
  async selectBookingOptions(page, bookingArgs) {
    try {
      ...
      if (!bikeTypeSelected) {
        ...
        // Fallback: try selecting first available option from any group (excluding CBT type and Full Licence groups)
        for (let groupIndex = 0; groupIndex < groupCount; groupIndex++) {
          const group = allGroups.nth(groupIndex);
          const groupHeading = group.locator('h1.jqx_formBoilerPlateText.jqx_formHeading span').first();
          const headingText = await groupHeading.textContent().catch(() => '');
