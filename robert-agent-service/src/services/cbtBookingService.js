...
class CBTBookingService extends BaseBookingService {
  constructor() {
    super(
      'CBT', // courseType
      'https://www.bookcbtnow.com/incontact/public/gateway.aspx?func_id=79A2A98E7C95DA57&obc_id=8ABD7DD7AAF3C2C5', // availabilityUrl
      './screenshots/cbt-booking' // screenshotsDir
    );
  }

  // Override executeExistingClientWorkflow to add email fallback logic
  ...
  async step8FillContactDetails(page, bookingArgs, callContext, screenshots, workflowType) {
    if (workflowType === 'existing') {
      // For existing clients, use lookupContactAndWait
      ...
      // For new clients, use base implementation
      await super.step8FillContactDetails(page, bookingArgs, callContext, screenshots, workflowType);
    }
  }

  // STEP 7: Select booking options for existing client (CBT-specific)
  ...
  // STEP 5: Select booking options for new client (CBT-specific)
  ...
  // Internal method: Select booking options (CBT-specific)
  async selectBookingOptions(page, bookingArgs) {
    try {
      ...
      for (let groupIndex = 0; groupIndex < groupCount; groupIndex++) {
        ...
        // STEP 2: Skip "Full Licence courses" group (not relevant for CBT)
        if (normalizedHeading.includes('full licence')) {
          console.log(`⏭️ [STEP 8] Skipping "Full Licence courses" group (not relevant for CBT)`);
          continue;
        }
