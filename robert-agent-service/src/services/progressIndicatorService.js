...
const TOOL_ACK_DELAY_MS = {
  ...
  // 500ms delay — these tools either have a fast path or complete faster than expected under load.
  // Without the delay the ack fires before toolExecutionCompleting is set, causing a response-lock
  // race that silences the agent. Same rationale as booking_step_send_payment_request below.
  booking_step_navigate_contacts:      500,
...
class ProgressIndicatorService {
  ...
  buildStrictHoldingInstruction(escapedMessage, toolName) {
    ...
    return `CRITICAL — SYSTEM HOLDING MESSAGE. While ${step} is running, you MUST output ONLY the exact phrase below. No other words. No summary of the booking. No confirmation. No "booking confirmed". No "booking is now being processed". No "everything is in place". No "you will receive a confirmation soon". No apology. No rephrasing. Do NOT mention: course name, date, time, location, session, payment, bike type, transfer, confirmation, "you're all set", contact details, full name, email, phone, or next steps. Do not ask for name, email, or phone. Do not ask any questions. Do not call any tools. If you cannot say only this phrase, say nothing. The ONLY phrase you are allowed to say is: "${escapedMessage}". Say that phrase and nothing else.`;
  }
